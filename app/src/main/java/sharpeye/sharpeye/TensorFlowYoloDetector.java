...
import sharpeye.sharpeye.env.SplitTimer;
      ...
      for (int x = 0; x < gridWidth; ++x) {
        for (int b = 0; b < NUM_BOXES_PER_BLOCK; ++b) {
          ...
          if (confidenceInClass > 0.01) {
            LOGGER.i(
                "%s (%d) %f %s", labels.get(detectedClass), detectedClass, confidenceInClass, rect);
            pq.add(new Recognition("" + offset, labels.get(detectedClass), confidenceInClass, rect));
          }
        }
      }
    }
